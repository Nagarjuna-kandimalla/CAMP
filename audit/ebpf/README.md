...
Workflow usage:
- `bowtie` and `minimap` now provide selective overlay configs that call `run_with_optional_audit.py` and consult the gating task plan before attaching eBPF
- `mcmicro` now provides an artifact-local selective overlay config that uses `run_with_optional_audit.py`, `task_plan_loader.py`, and `ebpf_audit.py`
